...
<div class={`group relative overflow-hidden rounded-xl border border-gray-200 p-4 shadow-sm transition-colors hover:bg-white/70 dark:border-gray-800 dark:hover:bg-gray-800 ${win === true ? 'bg-emerald-100/60 dark:bg-emerald-500/10' : win === false ? 'bg-rose-100/60 dark:bg-rose-500/10' : 'bg-gray-50 dark:bg-gray-900/30'}`}>
  ...
  <div class="grid grid-cols-[1fr_auto_1fr] grid-rows-[auto_auto] items-center gap-x-3 gap-y-2 md:gap-x-6">
    ...
    <div class="col-start-1 row-start-1 min-w-0 text-left">
      {#if to64(game.winner)}
        <a class={`block truncate md:text-lg ${isPlayerLeft ? 'font-extrabold text-gray-900 dark:text-gray-100' : 'font-medium text-gray-700 dark:text-gray-300'} hover:underline`} href={`/mge/games/${to64(game.winner)}`}>{game.winnername}</a>
      {:else}
    ...
    <!-- Center score -->
    <div class="col-start-2 row-start-1 row-span-2 justify-self-center self-center">
      <div class="flex flex-col items-center">
        <div class="text-2xl tracking-tight text-gray-900 md:text-3xl dark:text-gray-100">
    ...
    <div class="col-start-3 row-start-1 min-w-0 text-right">
      {#if to64(game.loser)}
        <a class={`block truncate md:text-lg ${isPlayerRight ? 'font-extrabold text-gray-900 dark:text-gray-100' : 'font-medium text-gray-700 dark:text-gray-300'} hover:underline`} href={`/mge/games/${to64(game.loser)}`}>{game.losername}</a>
      {:else}
