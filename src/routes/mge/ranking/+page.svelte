  ...
  async function fetchAvatarsForCurrentPage() {
    try {
      ...
      for (const r of ranking) {
        try {
          ids64.push(new ID(r.steamid).get64());
        } catch {}
      }
...
<div class="p-4">
  <div class="flex items-end justify-between gap-2">
    ...
    <div class="flex flex-wrap items-center gap-2">
      ...
      <input
        class="w-64 rounded-md border border-gray-200 bg-white px-2 py-1 text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200"
        ...
        {#each cardItems as r, idx}
          <LeaderboardCard
            player={{
              ...
              avatarUrl: (() => {
                try {
                  return avatarMap[new ID(r.steamid).get64()] ?? null;
                } catch {
                  return null;
                }
              })()
